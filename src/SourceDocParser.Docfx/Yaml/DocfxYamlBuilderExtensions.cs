...
/// Internal helpers let the emitter and its tests compose individual
/// page sections.
/// </summary>
internal static class DocfxYamlBuilderExtensions
{
    ...
    /// <summary>Extension members for <c>ApiMember</c>.</summary>
    /// <param name="member">Member whose docfx identifier to format.</param>
    extension(ApiMember member)
    {
        ...
        internal string MemberIdFor() => member.Name switch
        {
            ".ctor" => "#ctor",
            ".cctor" => "#cctor",
            _ => member.Name,
    ...
    /// <summary>Extension members for <c>ApiType</c>.</summary>
    /// <param name="type">Type whose member list to retrieve.</param>
    extension(ApiType type)
    {
        ...
        internal ApiMember[]? MembersOf() => type switch
        {
    ...
    /// <summary>Extension members for <c>ApiTypeReference</c>.</summary>
    /// <param name="reference">Reference whose YAML scalar to format.</param>
    extension(ApiTypeReference reference)
    {

        /// <summary>Returns the scalar docfx expects for a type reference.</summary>
        /// <returns>Stripped UID when present; otherwise the display name.</returns>
        internal string ReferenceScalar() =>
            reference is { Uid: [_, ..] uid } ? CommentIdPrefix.Strip(uid) : reference.DisplayName;
    }

    /// <summary>Extension members for <c>StringBuilder</c>.</summary>
    /// <param name="sb">Destination builder for the YAML page.</param>
    extension(StringBuilder sb)
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendTypeItem(ApiType type) =>
            sb.AppendTypeItem(type, DocfxCatalogIndexes.Empty, DefaultConverter);
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendTypeItem(ApiType type, DocfxCatalogIndexes indexes) =>
            sb.AppendTypeItem(type, indexes, DefaultConverter);
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendTypeItem(ApiType type, DocfxCatalogIndexes indexes, XmlDocToMarkdown converter)
        {
            _ = sb.Append("- uid: ").AppendScalar(CommentIdPrefix.Strip(type.Uid)).AppendLine()
                ...
                .AppendBlockScalar("  summary: ", converter.Convert(type.Documentation.Summary))
                .AppendBlockScalar("  remarks: ", converter.Convert(type.Documentation.Remarks))
                ...
                .AppendKindSpecificSyntax(type, converter)
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendMemberItems(ApiType type) =>
            sb.AppendMemberItems(type, DefaultConverter);

        /// <summary>
        /// Converter overload of <see cref="AppendMemberItems(StringBuilder, ApiType)"/>.
        /// Routes per-member docs through the supplied
        /// <see cref="XmlDocToMarkdown"/> at write time so the catalog
        /// stays raw.
        /// </summary>
        /// <param name="type">Type whose members to enumerate.</param>
        /// <param name="converter">XML to Markdown converter.</param>
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendMemberItems(ApiType type, XmlDocToMarkdown converter)
        {
            ...
            for (var i = 0; i < members.Length; i++)
            {
                ...
                _ = sb.AppendMemberItem(type, members[i], converter);
            }

            return sb;
        }

        /// <summary>Writes the page-level <c>references:</c> list. No-op when the list is empty.</summary>
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendPageReferences(
            ...
            HashSet<string> internalUids)
        {
            ...
            _ = sb.Append("references:\n");
            for (var i = 0; i < references.Length; i++)
            {
                _ = DocfxReferenceEnricher.AppendEnrichedReference(sb, references[i], internalUids);
            }
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendScalar(string value)
        {
            if (value is [])
            {
                return sb.Append("''");
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendChildren(ApiType type)
        {
            var members = MembersOf(type);
            ...
            _ = sb.Append("  children:\n");
            for (var i = 0; i < uids.Length; i++)
            {
                _ = sb.Append("  - ").AppendScalar(uids[i]).AppendLine();
            }
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendChild(ApiMember member) =>
            sb.Append("  - ").AppendScalar(CommentIdPrefix.Strip(member.Uid)).AppendLine();

        /// <summary>Writes the <c>namespace:</c> field, skipped for the global namespace.</summary>
        /// <param name="ns">Namespace text (empty for global).</param>
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendNamespace(string ns) =>
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendInheritance(ApiType type) => type switch
        {
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendBaseType(ApiTypeReference? baseRef) =>
            baseRef is not null
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendImplements(ApiTypeReference[] interfaces)
        {
            if (interfaces is [])
            {
                return sb;
            ...
            _ = sb.Append("  implements:\n");
            for (var i = 0; i < interfaces.Length; i++)
            {
                _ = sb.AppendInterface(interfaces[i]);
            }
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendInterface(ApiTypeReference iface) =>
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendDerivedClasses(ApiTypeReference[] derived)
        {
            if (derived is [])
            {
                return sb;
            ...
            _ = sb.Append("  derivedClasses:\n");
            for (var i = 0; i < derived.Length; i++)
            {
                var reference = derived[i];
                _ = sb.Append("  - ").AppendScalar(reference.Uid is [_, ..] uid ? CommentIdPrefix.Strip(uid) : reference.DisplayName).AppendLine();
            }
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendInheritedMembers(string[] inherited)
        {
            if (inherited is [])
            {
                return sb;
            ...
            _ = sb.Append("  inheritedMembers:\n");
            for (var i = 0; i < inherited.Length; i++)
            {
                _ = sb.Append("  - ").AppendScalar(inherited[i]).AppendLine();
            }
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendExtensionMethods(ApiMember[] extensions)
        {
            if (extensions is [])
            {
                return sb;
            ...
            _ = sb.Append("  extensionMethods:\n");
            for (var i = 0; i < extensions.Length; i++)
            {
                _ = sb.Append("  - ").AppendScalar(CommentIdPrefix.Strip(extensions[i].Uid)).AppendLine();
            }
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendExtensionBlocks(ApiExtensionBlock[] blocks)
        {
            if (blocks is [])
            {
                return sb;
            ...
            for (var i = 0; i < blocks.Length; i++)
            {
                ...
                _ = sb.Append("  - receiverName: ").AppendScalar(block.ReceiverName).AppendLine()
                    .Append("    receiverType: ").AppendScalar(receiverUid).AppendLine()
                    .Append("    members:\n");
                for (var m = 0; m < block.Members.Length; m++)
                {
                    _ = sb.Append("    - ").AppendScalar(CommentIdPrefix.Strip(block.Members[m].Uid)).AppendLine();
                }
            }
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendSeealso(string[] seealso)
        {
            if (seealso is [])
            {
                return sb;
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendKindSpecificSyntax(ApiType type) =>
            sb.AppendKindSpecificSyntax(type, DefaultConverter);

        /// <summary>Converter overload that threads <paramref name="converter"/> into kind-specific syntax helpers (e.g. enum value descriptions).</summary>
        /// <param name="type">Type to render.</param>
        /// <param name="converter">XML to Markdown converter.</param>
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendKindSpecificSyntax(ApiType type, XmlDocToMarkdown converter) => type switch
        {
            ApiEnumType e => sb.AppendEnumSyntax(e, converter),
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendObjectSyntax(ApiObjectType type) => sb
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendEnumSyntax(ApiEnumType type)
        {
            if (type.Values is [])
            {
                return sb;
            ...
            {
                _ = sb.AppendEnumValue(type.Values[i]);
            }

            return sb;
        }

        /// <summary>Converter overload that threads <paramref name="converter"/> into each enum value's description.</summary>
        /// <param name="type">Enum type to render.</param>
        /// <param name="converter">XML to Markdown converter for the value descriptions.</param>
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendEnumSyntax(ApiEnumType type, XmlDocToMarkdown converter)
        {
            if (type.Values is [])
            {
                return sb;
            ...
            for (var i = 0; i < type.Values.Length; i++)
            {
                _ = sb.AppendEnumValue(type.Values[i], converter);
            }
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendEnumValue(ApiEnumValue value) =>
            sb.AppendEnumValue(value, DefaultConverter);

        /// <summary>Converter overload of <see cref="AppendEnumValue(StringBuilder, ApiEnumValue)"/>.</summary>
        /// <param name="value">Enum value to emit.</param>
        /// <param name="converter">XML to Markdown converter for the description.</param>
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendEnumValue(ApiEnumValue value, XmlDocToMarkdown converter) => sb
            .Append("    - id: ").AppendScalar(value.Name).AppendLine()
            .Append("      defaultValue: ").AppendScalar(value.Value).AppendLine()
            .AppendBlockScalar("      description: ", converter.Convert(value.Documentation.Summary));
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendDelegateSyntax(ApiDelegateType type) => sb
            .Append(SyntaxBlockHeader)
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendMemberItem(ApiType type, ApiMember member) =>
            sb.AppendMemberItem(type, member, DefaultConverter);

        /// <summary>Converter overload of <see cref="AppendMemberItem(StringBuilder, ApiType, ApiMember)"/>.</summary>
        /// <param name="type">Containing type (provides parent UID + names).</param>
        /// <param name="member">Member to render.</param>
        /// <param name="converter">XML to Markdown converter.</param>
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendMemberItem(ApiType type, ApiMember member, XmlDocToMarkdown converter)
        {
            ...
            _ = sb.Append("- uid: ").AppendScalar(CommentIdPrefix.Strip(member.Uid)).AppendLine()
                ...
                .Append("  - ").AppendScalar(type.AssemblyName).AppendLine()
                .AppendNamespace(type.Namespace)
                .AppendBlockScalar("  summary: ", converter.Convert(member.Documentation.Summary))
                .AppendBlockScalar("  remarks: ", converter.Convert(member.Documentation.Remarks))
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendAttributes(ApiAttribute[] attributes)
        {
            ...
            {
                _ = sb.AppendAttributeEntry(filtered[i]);
            }
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendAttributeEntry(ApiAttribute attribute)
        {
            var typeRef = StripCommentIdPrefixIfPresent(attribute.Uid);
            _ = sb.Append("  - type: ").AppendScalar(typeRef).AppendLine();
            ...
            if (attribute.ConstructorUid is [_, ..] ctorUid)
            {
                ...
                _ = sb.Append("    ctor: ").AppendScalar(ctorScalar).AppendLine();
            }

            if (attribute.Arguments is [])
            {
                _ = sb.Append("    arguments: []\n");
                return sb;
            }
            ...
            for (var i = 0; i < attribute.Arguments.Length; i++)
            {
                ...
                {
                    _ = sb.Append("name: ").AppendScalar(name).AppendLine().Append("      ");
                }

                _ = sb.Append("value: ").AppendScalar(argument.Value).AppendLine();
            }
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendQualifiedScalar(string left, char separator, string right)
        {
            if (left is [])
            {
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendMemberSyntax(ApiMember member) => sb
            .Append(SyntaxBlockHeader)
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendParameters(ApiParameter[] parameters, string indent)
        {
            if (parameters is [])
            {
                return sb;
            ...
            _ = sb.Append(indent).Append("parameters:\n");
            for (var i = 0; i < parameters.Length; i++)
            {
                _ = sb.AppendParameter(parameters[i], indent);
            }
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendParameter(ApiParameter parameter, string indent) => sb
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendDefaultValue(string? defaultValue, string indent) =>
            defaultValue is { Length: > 0 } def
                ? sb.Append(indent).Append("  defaultValue: ").AppendScalar(def).AppendLine()
                : sb;

        /// <summary>Calls <see cref="AppendReturn"/> when <paramref name="returnType"/> is non-null. Lets the call sites stay flat.</summary>
        /// <param name="returnType">Optional return-type reference.</param>
        /// <param name="indent">Indent prefix.</param>
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendReturnIfPresent(ApiTypeReference? returnType, string indent) =>
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal StringBuilder AppendReturn(ApiTypeReference returnType, string indent) => sb
            .Append(indent).Append("return:\n").Append(indent).Append(TypeFieldLabel)
            .AppendScalar(ReferenceScalar(returnType))
            .AppendLine();

        /// <summary>Writes <paramref name="prefix"/> + <paramref name="value"/> when the value is non-empty; no-op otherwise. Keeps the page terse by dropping every empty-string field.</summary>
        /// <param name="prefix">Key + colon prefix.</param>
        /// <param name="value">Value to write (skipped when null/empty).</param>
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendIfPresent(string prefix, string? value) =>
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendBlockScalar(string prefix, string value)
        {
            if (value is [])
            {
                return sb;
        ...
        /// <summary>Writes a quoted scalar -- wraps the value in double quotes and escapes embedded characters per the YAML spec.</summary>
        /// <param name="value">Scalar to quote.</param>
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendQuotedScalar(string value)
        {
            _ = sb.Append('"');
            for (var i = 0; i < value.Length; i++)
            {
                _ = sb.AppendQuotedChar(value[i]);
            }
        ...
        /// <returns>The same <paramref name="sb"/>, for chaining.</returns>
        internal StringBuilder AppendQuotedChar(char c) => c switch
        {
            '"' => sb.Append("\\\""),
            '\\' => sb.Append(@"\\"),
            '\n' => sb.Append("\\n"),
    ...
    /// <param name="value">Documentation comment identifier to normalize.</param>
    extension(string value)
    {

        /// <summary>Removes an XML-doc comment-id prefix when present.</summary>
        /// <returns>The suffix after the prefix, or the original string when no prefix exists.</returns>
        internal string StripCommentIdPrefixIfPresent() =>
