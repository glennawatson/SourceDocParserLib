...
/// <summary>Represents a parsed Target Framework Moniker (TFM).</summary>
...
[System.Diagnostics.DebuggerDisplay("Tfm: {IsModernNet}")]
public sealed record Tfm(string Raw, string Family, string Version, string? Platform)
{
    /// <summary>Framework-family name for .NET Standard.</summary>
    private const string NetStandardFamily = "netstandard";

    /// <summary>The length of the "netstandard" prefix.</summary>
    private const int NetStandardPrefixLength = 11;

    /// <summary>The length of the "net" prefix.</summary>
    private const int NetPrefixLength = 3;

    /// <summary>The base rank for modern .NET TFMs.</summary>
    private const int ModernNetBaseRank = 550;

    /// <summary>The rank for .NET Standard 2.1.</summary>
    private const int NetStandard21Rank = 510;

    /// <summary>The rank for .NET Standard 2.0.</summary>
    private const int NetStandard20Rank = 500;

    /// <summary>The default rank for .NET Standard.</summary>
    private const int NetStandardDefaultRank = 480;

    /// <summary>The rank for .NET Framework.</summary>
    private const int NetFrameworkRank = 300;

    /// <summary>The default rank for unknown TFMs.</summary>
    private const int DefaultRank = 100;

    /// <summary>The multiplier used to convert modern .NET versions to ranks.</summary>
    private const int ModernNetVersionRankMultiplier = 100;

    /// <summary>The adjustment to the rank for platform-specific TFMs.</summary>
    private const int PlatformAdjustment = -5;

    /// <summary>Parsed TFM cache to reduce allocations.</summary>
    ...
    /// <summary>Gets a value indicating whether this is a modern .NET TFM (net5.0+).</summary>
    public bool IsModernNet => Family == "net" && Version.AsSpan() is ['5', ..] or { Length: > 1 };

    /// <summary>Gets a value indicating whether this is a .NET Framework TFM.</summary>
    public bool IsNetFramework => Family == "net4";

    /// <summary>Gets a value indicating whether this is a .NET Standard TFM.</summary>
    public bool IsNetStandard => Family == NetStandardFamily;

    /// <summary>Gets the pre-calculated rank for this TFM.</summary>
    public int Rank { get; init; }

    /// <summary>Parses a raw TFM string into a <see cref="Tfm"/> record.</summary>
    ...
    public static Tfm Parse(string tfm)
    {
        ...
        return _cache.GetOrAdd(tfm, static key =>
        {
            ...
            {
                _ when baseTfm.StartsWith(NetStandardFamily, StringComparison.OrdinalIgnoreCase) => (NetStandardFamily, new(baseTfm[NetStandardPrefixLength..])),
    ...
    /// <summary>Calculates a numeric rank for a TFM to determine priority.</summary>
