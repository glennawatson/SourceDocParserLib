...
/// <summary>Per-resolver cache of <see cref="ApiDocumentation"/> instances keyed by <see cref="ISymbol"/>.</summary>
/// <remarks>
...
internal sealed class DocResolveCache
{
    ...
    /// <summary>Gets the cached documentation for <paramref name="symbol"/>, creating and storing it on a cache miss.</summary>
    ...
    internal ApiDocumentation GetOrAdd<TState>(
        ISymbol symbol,
        TState state,
        Func<ISymbol, TState, ApiDocumentation> builder)
    {
        ...
        slot = ApiDocumentation.Empty;
        try
        {
            var documentation = builder(symbol, state);

            // Recursive resolution can resize the dictionary, invalidating the original slot reference.
            _bySymbol[symbol] = documentation;
            return documentation;
        }
        catch
        {
            _ = _bySymbol.Remove(symbol);
            throw;
        }
    }
}
