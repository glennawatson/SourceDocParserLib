...
/// <summary>Pins <see cref="Readers.PackageSourceCredentialsReader"/> on the with-creds fixture (env-var expansion of GitHub PAT) plus inline XML for the spaces-in-source-name case.</summary>
public class PackageSourceCredentialsReaderTests
{
    /// <summary>Fixture value for GithubToken.</summary>
    private const string GithubToken = "GITHUB_TOKEN";

    /// <summary>The fixture's <c>github</c> source carries <c>%GITHUB_TOKEN%</c>; with that env var set the reader returns the expanded value.</summary>
    ...
    public async Task ExpandsEnvVarInClearTextPassword()
    {
        ...
        var original = Environment.GetEnvironmentVariable(GithubToken);

        try
        {
            Environment.SetEnvironmentVariable(GithubToken, "ghp_secret_value_xyz");
        ...
        {
            Environment.SetEnvironmentVariable(GithubToken, original);
        }
    }

    /// <summary>Source names with spaces use NuGet's <c>_x0020_</c> XML escape -- the reader unescapes back to the friendly name.</summary>
    /// <returns>A task representing the test execution.</returns>
    [Test]
    ...
    /// <summary>A source block missing the password is skipped -- partial credentials are not surfaced.</summary>
    /// <returns>A task representing the test execution.</returns>
    [Test]
