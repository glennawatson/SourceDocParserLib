...
/// <summary>Direct tests for the small per-scope cache types that back the main-library hot paths.</summary>
public class ScopedCacheTests
{
    /// <summary>The doc resolver cache builds once per symbol and reuses the cached instance thereafter.</summary>
    ...
    /// <summary>The namespace display-name cache formats once and then reuses the cached string for the same symbol.</summary>
    /// <returns>A task representing the test execution.</returns>
    [Test]
    ...
    /// <summary>Builds an in-memory <see cref="CSharpCompilation"/> from <paramref name="source"/>.</summary>
    ...
    /// <summary>Small mutable callback wrapper so the static cache-miss lambdas can increment a local counter.</summary>
    /// <param name="callback">Action to run on increment.</param>
    private sealed class Counter(Action callback)
    {
        /// <summary>Runs the wrapped callback.</summary>
