...
/// <summary>Tests for <see cref="XmlDocSource"/> -- the indexed view over a .NET XML doc file produced by the hand-rolled forward scanner.</summary>
public class XmlDocSourceTests
{
    /// <summary>Expected fixture value used by FromStringIndexesMultipleMembers.</summary>
    private const int FromStringIndexesMultipleMembersExpectedValue = 3;
    ...
    public async Task FromStringIndexesMultipleMembers()
    {
        ...
        await Assert.That(source.Count).IsEqualTo(FromStringIndexesMultipleMembersExpectedValue);
