...
public class AttributeConstructorUidWalkerTests
{
    ...
    public async Task NoArgAttributeCapturesParameterlessCtorUid()
    {
        ...
        var target = ((ApiObjectType)(await Assert.That(catalog.Types).HasSingleItem(static t => t is ApiObjectType && (t.Name == "Target"))));
        var serializable = (await Assert.That(target.Attributes).HasSingleItem(static a => a.DisplayName == "Serializable"));
    ...
    public async Task ParameterisedAttributeCapturesBoundCtorUid()
    {
        ...
        var catalog = walker.Walk("net10.0", compilation.Assembly, compilation, resolver);
        var target = ((ApiObjectType)(await Assert.That(catalog.Types).HasSingleItem(static t => t is ApiObjectType && (t.Name == "Target"))));
        var browsable = (await Assert.That(target.Attributes).HasSingleItem(static a => a.DisplayName == "Browsable"));
